...
# --- Added for Google Sheets integration ---
...
class SalesforceEnricher:
    def __init__(self):
        """Initialize the Salesforce enricher with Google Sheets data."""
        ...
        self.dfs = self.get_gsheet_dataframes(self.gs_url, self.creds_path)
        print(f"Loaded Google Sheets: {list(self.dfs.keys())}")
        
        # Initialize country mapping
        self.country_map = self._initialize_country_mapping()
    ...
    def _initialize_country_mapping(self):
        """Initialize country mapping from Google Sheets."""
        country_df = self.dfs.get('country')
        country_map = {}
    ...
    def update_subscription_names_from_country(self, subscriptions):
        """
        ...
        for item in subscriptions:
            ...
            if pname in self.country_map:
                print(f"[Country Mapping] ProductName replaced: '{pname}' -> '{self.country_map[pname]}'")
                item['ProductName'] = self.country_map[pname]
                item['Note_country_mapping'] = f"Amendment: ProductName replaced from '{pname}' to '{self.country_map[pname]}' via country sheet."

    def get_all_records(self, json_data, name):
        """Get all records from a specific section in the JSON data."""
    ...
    def fetch_product2_fields(self, product_names, batch_size=100):
        """Fetch Product2 fields from Salesforce."""
        ...
        for item in subscriptions:
            ...
            if not recs:
                ...
                for f in extra_fields: 
                    item[f] = None
                item["Note"] = "Amendment: No exact match found based on Subscription Name"
            else:
                ...
                if len(recs) > 1:
                    item["Note"] = "Amendment: Duplicate Subscription Names found, Id was taken from latest CreatedDate"
                else:
                    item["Note"] = "Amendment: Successfully Matched"
        return subext_to_pid

    # ======= Functions for lis ========
    def get_lineitem_keys(self, lineitems, subext_to_pid):
        """Get (Parent ProductId, lisName) pairs needed for querying options."""
        keymap = {}
    ...
    def fetch_option_product_ids(self, keymap, batch_size=50):
        """Fetch option product IDs from Salesforce."""
        if not keymap: 
            return {}
    ...
    def update_lineitem_product_ids(self, lineitems, keymap, option_pid_map):
        """Update line item fields with option product data."""
        for (parent_pid, lis_name), item in keymap.items():
            ...
            if not records:
                ...
                item['OwnerId'] = None
                item['Note'] = "Amendment: No exact match found based on Line Item Source Name"
            else:
                ...
                item['Note'] = "Amendment: Duplicate Line Item Source Names found, Id was taken from latest CreatedDate" if len(records) > 1 else "Amendment: Successfully Matched"

    def update_lisnames_from_sku_sheet(self, lineitems, subscriptions, subext_to_pid):
        """Update lisName using SKU sheet mapping."""
        sku_df = self.dfs.get('SKU')
        if sku_df is not None and sku_df.shape[1] >= 4:
            ...
            for item in lineitems:
                ...
                if parent_pname and lis_name and (parent_pname, lis_name) in sku_map:
                    ...
                    item["Note_SKU_mapping"] = f"Amendment: lisName replaced from '{old_lis}' to '{item['lisName']}' via SKU sheet."
                    print(f"[SKU Mapping] lisName replaced for ProductName '{parent_pname}': '{old_lis}' -> '{item['lisName']}'")

    # ======= Functions for Sub Consumption Schedule ========
    def fetch_product_consumption_schedules(self, product_ids, batch_size=100):
        """Fetch product consumption schedules from Salesforce."""
        if not product_ids:
            return {}
    ...
    def fetch_consumption_schedules(self, subscription_names, batch_size=100):
        """Fetch consumption schedules from Salesforce."""
    ...
    def update_sub_consumption_schedules(self, schedules, subext_to_pid, pcs_map, consumption_schedule_map):
        """Update subscription consumption schedules with Salesforce data."""
    ...
    # ======= Functions for Sub Consumption Rate ========
    def fetch_consumption_rates(self, subcs_ids, batch_size=100):
        """Fetch consumption rates from Salesforce."""
        if not subcs_ids:
            return {}
    ...
    def update_sub_consumption_rates(self, rates, subext_to_subcsid, consumption_rate_map):
        """Update subscription consumption rates with Salesforce data."""
        for item in rates:
            sub_ext_id = item.get("subExternalId")
    ...
    def calculate_summary_stats(self, data):
        """Calculate summary statistics and matching rates."""
        ...
        data["MatchedSubCnt"] = sum(
            1 for item in subscriptions if (
                item.get("Note") == "Amendment: Successfully Matched" or
                item.get("Note") == "Amendment: Duplicate Subscription Names found, Id was taken from latest CreatedDate"
            )
        )
        data["MatchedLisCnt"] = sum(
            1 for item in lineitems if (
                item.get("Note") == "Amendment: Successfully Matched" or
                item.get("Note") == "Amendment: Duplicate Line Item Source Names found, Id was taken from latest CreatedDate"
            )
        )
    ...
    def enrich_contract_data(self, data):
        """
        Main method to enrich contract data with Salesforce information.
        ...
        print("Processing subscription consumption rates...")
        ...
        consumption_rate_map = self.fetch_consumption_rates(subcs_ids)
        ...
        data = self.calculate_summary_stats(data)
        
        print("Salesforce enrichment completed successfully!")
        return data
...
#     # Test the class
#     input_json_path = "BACKEND/THIRDV/output_single_json/Constellation_Payments_-_KYB_PM_-_USA-Trulioo-16968-12-Feb-2025-19-7-13-signed.json"
#     output_json_path = input_json_path.replace('.json', '_final.json')
...
#     # Create enricher instance and process data
#     enricher = SalesforceEnricher()
#     enriched_data = enricher.enrich_contract_data(input_data)

#     # Save enriched/final JSON
#     with open(output_json_path, "w", encoding="utf-8") as f:
#         json.dump(enriched_data, f, indent=2)
#     print(f"Processed and saved: {output_json_path}")

#     # Save each dataframe as CSV
#     for section in enriched_data.get("output_records", []):
#         name = section.get("name")
#         records = section.get("data", [])
#         if records:
#             df = pd.DataFrame(records)
#             csv_path = os.path.join(csv_folder, f"{os.path.splitext(os.path.basename(input_json_path))[0]}_{name}.csv")
