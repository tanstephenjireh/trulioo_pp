...
class KYBExtractor:
    def __init__(self):
        """Initialize the KYB (Business Verification) extractor with environment and Salesforce connection."""
        # ========== ENV SETUP ==========
...
{full_text}
        ...
        for doc in outputs:
            if "subscription" in doc:
                for idx, subscription in enumerate(doc["subscription"], 1):
                    # Use amd_kyb prefix for external IDs
                    subExternalId = f"amd_kyb_sub_{idx}_{contractExternalId}"
                    subCsExternalId = f"amd_kyb_subcs_{idx}_{contractExternalId}"
                    ...
                    for scr in subscription.get("scr", []):
                        subCrExternalId = f"amd_kyb_subcr_{idx}_{contractExternalId}"
                        sub_cr_flat = {
        ...
        for sub in subscriptions:
            ...
            if record:
                ...
                if len([r for r in product_map.values() if r['Name'] == name]) > 1:
                    sub["Note"] = "Amendment: Duplicates found, extracted latest CreatedDate"
                else:
    ...
    async def extract_kyb_data(self, markdown_text, contract_json):
        """
        Main method to extract Business Verification data from markdown and enrich contract JSON.
        """
        if not isinstance(contract_json, dict):
            raise TypeError("contract_json must be a Python dict (not a file path).")

        print("Starting Business Verification extraction process...")
        ...
        for pname, sub in llm_sub_dict.items():
            ...
            if not records:
                ...
                for field in [
                    ...
                    sub[field] = ""
                sub["Note"] = f"Amendment: Product not found based on ProductName: {pname}"
            else:
                ...
                for field in [
                    ...
                    sub[field] = record.get(field, "")
                if len(records) > 1:
                    sub["Note"] = f"Amendment: Duplicates found for ProductName: {pname}, Id taken from latest CreatedDate"
                else:
                    sub["Note"] = "Amendment: Successfully Matched"
        ...
        for pname, records in salesforce_records_by_name.items():
            if pname not in llm_sub_dict:
                ...
                record = max(records, key=lambda r: r.get("CreatedDate", ""))
                new_sub = {
                    "subExternalId": f"amd_kyb_sub_{next_index}_{contractExternalId}",
                    "ProductName": pname,
                    ...
                    "Note": "Amendment: From Salesforce query"
                }
                for field in [
                    "SBQQ__PricingMethod__c",
                    "SBQQ__BillingFrequency__c",
        ...
        for i, sub in enumerate(combined_subscriptions, 1):
            ...
            sub_cs = {
                "subCsExternalId": f"amd_kyb_subcs_{i}_{contractExternalId}",
            ...
            sub_cr = {
                "subCrExternalId": f"amd_kyb_subcr_{i}_{contractExternalId}",
        ...
        for i, r in enumerate(option_records.values(), 1):
            ...
            note = "Amendment: Successfully Matched"
            lineitem = {
                "lisExternalId": f"amd_kyb_lis{i}_{subext}",
        ...
        for lis in lineitem_sources:
            lis_consumption_schedule = {
                "scsExternalId": f"amd_kyb_scs_{lis['lisExternalId']}",
        ...
        for lis in lineitem_sources:
            lis_consumption_rate = {
                "scrExternalId": f"amd_kyb_scr_{lis['lisExternalId']}",
        ...
        print("Business Verification extraction completed successfully!")
        return merged
...
#     merged_json = extractor.extract_kyb_data(full_text, contract_json)
    
#     # Save merged JSON to a new file
#     import os
#     base, ext = os.path.splitext(input_contract_json)
#     output_json_path = f"{base}_with_amd_kyb{ext}"
