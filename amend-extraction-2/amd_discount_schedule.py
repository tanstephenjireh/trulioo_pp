...
class DiscountScheduleExtractor:
    def __init__(self):
        """Initialize the Discount Schedule extractor with environment and OpenAI connection."""
        # ========== ENV SETUP ==========
...
{full_text}
    ...
    async def extract_discount_sections_from_llm(self, docv_chunk):
        """Extract discount sections data from the chunk using LLM."""
        outputs = []
        if docv_chunk.strip():
            try:
                # Add delay before API call
    ...
    def merge_discount_schedule_into_json(self, input_json, discountSchedule):
        ...
        if person_match_discounts:
            pm_disc_ext_id = f"amd_pm_disc_{contract_id}"
            discount_schedule_records.append({
                "DiscExtId": pm_disc_ext_id,
                "ContractId": contract_id,
                "DiscountUnit": "Percent",
                "Type": "Step",
                "Description": "Amendment: Default person match discount schedule"
            })
            
            for i, tier in enumerate(person_match_discounts, 1):
                discount_rate_records.append({
                    "DiscExtId": pm_disc_ext_id,
                    "DiscRateExtId": f"amd_pm_disc_{i}_{contract_id}",
        ...
        if business_verification_discounts:
            bv_disc_ext_id = f"amd_bv_disc_{contract_id}"
            discount_schedule_records.append({
                "DiscExtId": bv_disc_ext_id,
                "ContractId": contract_id,
                "DiscountUnit": "Percent",
                "Type": "Step",
                "Description": "Amendment: Default business verification discount schedule"
            })
            ...
            for i, tier in enumerate(business_verification_discounts, 1):
                discount_rate_records.append({
                    "DiscExtId": bv_disc_ext_id,
                    "DiscRateExtId": f"amd_bv_disc_{i}_{contract_id}",
                    "InvoiceScheduleTierName": tier.get("InvoiceScheduleTierName", "NA"),
                    "LowerBound": tier.get("LowerBound", "NA"),
                    "UpperBound": tier.get("UpperBound", "NA"),
                    "Discount (%)": tier.get("Discount", "NA")
        ...
        if discount_schedule_records:
            ...
            for record in discount_schedule_records:
                if record.get("Description", "").startswith("Amendment: Default person match"):
                    pm_disc_ext_id = record.get("DiscExtId")
                elif record.get("Description", "").startswith("Amendment: Default business verification"):
                ...
                if record.get('name') == 'Subscription' and isinstance(record.get('data'), list):
                    for item in record['data']:
                        if isinstance(item, dict):
                            ...
                            if sub_external_id.startswith('amd_sub') and pm_disc_ext_id:
                                item['DiscExtId'] = pm_disc_ext_id
                            elif sub_external_id.startswith('amd_kyb') and bv_disc_ext_id:
    ...
    async def extract_discount_schedule_data(self, markdown_text, contract_json):
        """
        Main method to extract discount schedule data from markdown and enrich contract JSON.
        """
        if not isinstance(contract_json, dict):
            raise TypeError("contract_json must be a Python dict (not a file path).")

        print("Starting Discount Schedule extraction process...")
        
        pmdiscount_boundaries = await self.call_llm_for_discount_sections_boundaries(markdown_text)
        print("\n==== PERSON MATCH DISCOUNT SCHEDULE BOUNDARIES LLM OUTPUT ====")
        print(json.dumps(pmdiscount_boundaries, indent=2, ensure_ascii=False))

        pmdiscount_chunks = await self.extract_full_disc_chunk_by_llm(markdown_text)
        ...
        if pmdiscount_chunks:
            ...
            print("Discount Schedule extraction completed successfully!")
            return merged_json
        else:
...
#     extractor = DiscountScheduleExtractor()
#     merged_json = extractor.extract_discount_schedule_data(full_text, contract_json)
    
#     # Save merged JSON to a new file
#     import os
#     base, ext = os.path.splitext(input_contract_json)
#     output_json_path = f"{base}_with_amd_discount{ext}"
#     with open(output_json_path, "w", encoding="utf-8") as f:
#         json.dump(merged_json, f, indent=2, ensure_ascii=False)
#     print(f"\n==== Saved merged JSON to: {output_json_path} ====") 