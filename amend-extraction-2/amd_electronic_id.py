...
class ElectronicIDExtractor:
    def __init__(self):
        """Initialize the Electronic Identification extractor with environment and Salesforce connection."""
        # ========== ENV SETUP ==========
...
"""
    ...
    async def call_llm_for_eid_boundaries(self, full_text):
        """Find the start and end line of the Electronic Identification section using LLM."""
        system_prompt = (
...
{full_text}
    ...
    async def extract_full_eid_chunk_by_llm(self, full_text):
        """Extract Electronic Identification chunk using LLM boundary detection."""
    ...
    async def extract_eid_consumption_from_llm(self, chunk):
        """Extract Electronic Identification consumption data from the chunk using LLM."""
        outputs = []
        if chunk.strip():
            try:
                # Add delay before API call
        ...
        for doc in outputs:
            if "subscription" in doc:
                for idx, subscription in enumerate(doc["subscription"], 1):
                    # Use amd_eid prefix for external IDs
                    subExternalId = f"amd_eid_sub_{contractExternalId}"
                    subCsExternalId = f"amd_eid_subcs_{contractExternalId}"
                    sub_flat = {
                    ...
                    for i, scr in enumerate(subscription.get("scr", []), 1):
                        subCrExternalId = f"amd_eid_subcr{i}_{contractExternalId}"
                        sub_cr_flat = {
    ...
    def merge_into_contract_json(self, contract_json, extracted_output):
        """Merge extracted Electronic Identification data into the contract JSON."""
    ...
    def update_subscription_product_fields(self, subscriptions, product_field_map, note_map=None):
        """Update subscription fields with Product2 Salesforce data."""
        extra_fields = [
        ...
        for sub in subscriptions:
            ...
            if records:
                ...
                if len(records) > 1:
                    sub["Note"] = "Amendment: Duplicates found. Id obtained from latest CreatedDate"
                else:
    ...
    async def extract_electronic_id_data(self, markdown_text, contract_json):
        """
        Main method to extract Electronic Identification data from markdown and enrich contract JSON.
        """
        if not isinstance(contract_json, dict):
            raise TypeError("contract_json must be a Python dict (not a file path).")

        print("Starting Electronic Identification extraction process...")
        ...
        print("Electronic Identification extraction completed successfully!")
        return merged


# if __name__ == "__main__":
#     # Test the class
#     md_path = "BACKEND/THIRDV/output_single_json/parsed_Services Agreement with Marqeta Inc (670ff63b27).md"
#     json_path = "BACKEND/THIRDV/output_single_json/Services Agreement with Marqeta Inc (670ff63b27).json"
#     output_dir = "BACKEND/THIRDV/output_single_json"

#     base_name = os.path.splitext(os.path.basename(json_path))[0]
#     output_path = os.path.join(output_dir, f"amd_electronicid_{base_name}.json")
...
#     extractor = ElectronicIDExtractor()
#     merged = extractor.extract_electronic_id_data(markdown_text, contract_json)

#     with open(output_path, "w", encoding="utf-8") as f:
#         json.dump(merged, f, indent=2, ensure_ascii=False)

#     print(f"✅ Merged Electronic ID data saved to {output_path}") 